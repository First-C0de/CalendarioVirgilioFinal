...
function modais(dias) {
    dias.addEventListener("click", function() {
        ...
        if (eventosExist.length > 0) {
            ...
            const botaocreatemais = document.createElement('button');
            botaocreatemais.innerText = 'Criar novo evento';
            botaocreatemais.classList.add('botaocriardiferente');
            divBotoes.appendChild(botaocreatemais);
            
            botaocreatemais.addEventListener('click', function() {
                modal.remove();
                // Abre o modal2 para criar um novo evento
                const modal2 = document.createElement('div');
                modal2.classList.add('modal2');
                const conteudo = document.querySelector('.conteudo');
                conteudo.appendChild(modal2);
                ...
                botaocreate2.addEventListener('click', function() {
                    const novoEvento = input.value.trim();
                    if (novoEvento) {
                        eventosExist.push({ eventos: novoEvento, dia: diaSelecionado });
                        localStorage.setItem(key, JSON.stringify(eventosExist));
                        adicionarumatarefa.innerHTML = eventosExist.map(evento => evento.eventos).join("<br>");
                        input.value = '';
                        modal2.remove();
